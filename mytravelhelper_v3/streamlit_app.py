...
st.set_page_config(
    page_title="MyTravelHelper — AI Assistant v3.0",
    page_icon="🌏",
    layout="wide",
...
with st.sidebar:
    st.markdown(
        ...
        <div style='text-align: center; margin-bottom: 20px;'>
            ...
            <p style='color:#8A90A6; font-size:0.85rem;'>Phiên bản 3.0.0 (Payments & Auth)</p>
        </div>
        """, 
        unsafe_allow_html=True
    )
    
    # Firebase Auth state in sidebar
    st.header("Tài khoản người dùng")
    if "user_info" in st.session_state and st.session_state["user_info"]:
        user = st.session_state["user_info"]
        st.success(f"👋 Xin chào, **{user.get('display_name', user.get('email'))}**")
        if st.button("Đăng xuất", type="secondary", key="btn_logout_sidebar"):
            st.session_state["user_info"] = None
            st.session_state["user_token"] = None
            st.success("Đã đăng xuất thành công!")
            st.rerun()
    else:
        st.info("🔑 Bạn chưa đăng nhập. Hãy đi tới mục **1 Auth** để đăng nhập/đăng ký.")
    ...
    # API Status
    if api_status["ok"]:
        st.success(f"**Chế độ NLU:** {api_status['mode']}\n\n{api_status['message']}")
    else:
        st.warning(f"**Chế độ NLU:** {api_status['mode']}\n\n{api_status['message']}")
        
    st.divider()
    
    st.subheader("Mô hình sử dụng (v3.0)")
    st.markdown(
        """
        - **Thanh toán:** VNPAY Sandbox (v2.1.0)
        ...
        - **Topic Clustering:** `BERTopic` / `KMeans`
        """
    )
