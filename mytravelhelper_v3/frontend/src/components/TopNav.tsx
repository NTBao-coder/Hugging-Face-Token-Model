...
export function TopNav({ user, onLoginClick, onLogout }: TopNavProps) {
    ...
    <header className="sticky top-0 z-40 h-[80px] w-full border-b border-hairline bg-canvas px-6 md:px-12 flex items-center justify-between text-ink select-none">
      ...
      <div className="flex items-center gap-1.5 cursor-pointer">
        <svg
          ...
          aria-label="MyTravelHelper homepage"
          className="h-[32px] w-[32px] fill-ink"
        >
        ...
        <span className="hidden lg:block text-ink font-bold text-[20px] tracking-tight">mytravelhelper</span>
      </div>
      ...
      <nav className="flex items-center gap-6 md:gap-8">
        <button
          ...
          className={[
            "relative pb-1.5 text-[15px] font-semibold transition-colors duration-150",
        ...
        >
          ...
          <span className="absolute -top-1.5 -right-6 rounded-full bg-primary px-1.5 py-0.5 text-[8px] font-bold text-white uppercase tracking-wider scale-90">
            NEW
          </span>
        </button>
      ...
      <div className="flex items-center gap-4">
        ...
        <div ref={dropdownRef} className="relative">
          <div
            ...
            className="flex items-center gap-3 border border-hairline rounded-full pl-3 pr-1.5 py-1.5 hover:shadow-md cursor-pointer transition-all bg-canvas"
          >
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-md bg-canvas shadow-lg border border-hairline py-1 z-50 text-ink">
              {user ? (
                <>
                  <div className="px-4 py-2 border-b border-hairline">
                    <p className="text-[13px] font-bold truncate">{user.displayName || "Khách"}</p>
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-[13px] text-red-500 hover:bg-surface-soft flex items-center gap-2 font-medium"
                  >
                ...
                <>
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-[13px] font-semibold hover:bg-surface-soft"
                  >
                    Đăng nhập / Đăng ký
                  </button>
                </>
